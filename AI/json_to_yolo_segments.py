...
COMMON_IMAGE_EXTENSIONS = ['.jpeg', '.jpg', '.png', '.bmp', '.tiff', '.webp']  # Add more if needed
...
def find_image_for_json(json_file_path):
    """
    Assumes image is in the same folder as the JSON, with the same base name.
    ...
    for ext in COMMON_IMAGE_EXTENSIONS:
        image_path = os.path.join(json_dir, f"{json_base_name}{ext}")
...
def convert_and_save_yolo_segmentation(json_data, output_label_path):
    ...
    if not all([image_width, image_height]):
        print(f"Error: Missing imageWidth or imageHeight in JSON for {os.path.basename(output_label_path)}")
        return False
    ...
    for shape in shapes:
        ...
        if label not in CLASS_TO_INDEX:
            print(f"Warning: Label '{label}' not in CLASS_NAMES. Skipping for {os.path.basename(output_label_path)}.")
            continue
        ...
        for point in points:
            if len(point) == 2:
                ...
                # print(f"Warning: Invalid point format {point} for label '{label}' in {os.path.basename(output_label_path)}")
                continue
...
if __name__ == "__main__":
    ...
    for split_name, json_file_list in files_splits.items():
        ...
        for json_file_path in tqdm(json_file_list):
            ...
            source_image_path = find_image_for_json(json_file_path)

            if not source_image_path:
                print(f"Warning: No corresponding image found for {json_file_path}. Skipping.")
                continue

            # Determine destination paths
            image_base_name = os.path.basename(source_image_path)
