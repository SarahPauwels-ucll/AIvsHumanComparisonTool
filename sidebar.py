...
def load_sidebar():
    ...
    profile_number_input = st.sidebar.text_input("Profile number", value=st.session_state.profile_number, key="profile_number")
    ...
    if profile_number_input.isdigit():
        ...
        st.sidebar.error("Profile number must be an number.")
    ...
    gender = st.sidebar.radio("Gender", ["Male", "Female"], index=0 if st.session_state.gender == "Male" else 1, key="gender")
    ...
    age = st.sidebar.number_input("Select age", step=1, min_value=0, value=st.session_state.age, key="age")
    if age != stored_age:
